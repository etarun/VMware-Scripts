...
# vCO OVF
VCO_OVA=VMware-vCO-Appliance-5.5.2.1-2179237_OVF10.ova  #path to OVA file

# e.g. 172.30.0.141/24 
VCO_DISPLAY_NAME=vCO-importPlugin
...
deployvCOOVA() {
	...
	${OVFTOOl_BIN}  --acceptAllEulas --ipAllocationPolicy=fixedPolicy --powerOn "--net:Network 1=${VCO_PORTGROUP}" --datastore=${VCO_DATASTORE} --diskMode=${VCO_DISK_TYPE} --name=${VCO_DISPLAY_NAME} --prop:varoot-password=infoblox --prop:vcoconf-password=infoblox --prop:vami.hostname=${VCO_HOSTNAME} --prop:vami.DNS.VMware_vCenter_Orchestrator_Appliance=${VCO_DNS} --prop:vami.gateway.VMware_vCenter_Orchestrator_Appliance=${VCO_GATEWAY} --prop:vami.ip0.VMware_vCenter_Orchestrator_Appliance=${VCO_IPADDRESS} --prop:vami.netmask0.VMware_vCenter_Orchestrator_Appliance=${VCO_NETMASK} ${VCO_OVA} vi://${VCENTER_USERNAME}:${VCENTER_PASSWORD}@${VCENTER_HOSTNAME}/?dns=${ESXI_HOSTNAME}
}
